...
data_2011_no_factor <- data.frame(lapply(data_SCI_2011, as.character), stringsAsFactors=FALSE)
rm(data_2011)
rm(data_2015)

write.table(data_2011_no_factor, "data_2011.csv",sep = ";", row.names=F)
write.table(data_2015_no_factor, "data_2015.csv",sep = ";", row.names=F)

data_no_factor <- rbind.fill(data_2015_no_factor,data_2011_no_factor)
...
responses_e_dich[responses_e_dich  == "CORRECT RESP_duplicated_29"] <- "CORRECT"
...
responses_e_dich[responses_e_dich == "PARTIALLY CORRECT RESP_duplicated_19"] <- "NOT DICH"
responses_e_dich[responses_e_dich == "PARTIALLY CORRECT RESP_duplicated_11"] <- "NOT DICH"
...
summary(coef(model_3pl, simplify=T, printSE=T)$items)
summary(coef(model_2pl, simplify=T, printSE=T)$items)

coef(model_3pl, simplify=F,printSE=T)
write.table(coef(model_3pl, simplify=T)$items,"TIMSS2011_2015_SCI_WORLD_3PL.csv")
